...
sudo cp /tmp/setup_cob4/upstart/cob-start /usr/sbin/cob-start
...
sudo cp /tmp/setup_cob4/scripts/cob-stop /usr/sbin/cob-stop
...
sudo cp /tmp/setup_cob4/scripts/cob-command /usr/sbin/cob-command
sudo echo "%users ALL=NOPASSWD:/usr/sbin/cob-command" | sudo tee -a /etc/sudoers

sudo cp /tmp/setup_cob4/scripts/tmux_cob /usr/sbin/tmux_cob
sudo echo "%users ALL=NOPASSWD:/usr/sbin/tmux_cob" | sudo tee -a /etc/sudoers
